...
void loop() {
  ...
  if(cnt>-1) {
    ...
    Serial.print(buf); //Serial.write(buf), 블루투스 측 내용을 시리얼 모니터에 출력
    BTSerial.print(buf); // BTSerial.write(buf), 시리얼 모니터 내용을 블루투스 측에 write
...
void serialEvent() {
  while (Serial.available()) {
    ...
    if (inChar == 'a' || inChar=='1') {
      cnt = 0;
      digitalWrite(13, HIGH);
    }
    else if (inChar == 'b'  || inChar=='0') {
      cnt = -1;
      digitalWrite(13, LOW);
    }
  }
